...
while ($row2 = mysqli_fetch_assoc($empRecords2)) 
	...
	//insert
	$xyz = md5("$i_tglku");
	mysqli_query($koneksi, "INSERT INTO perilaku_lap_tipe_laporan(kd, tanggal, postdate) VALUES 
								('$xyz', '$i_tglku', '$today')");


	
	//membaca semua data, trus masukin ke satu table
	$empQuery = "SELECT * FROM e_m_tipe_laporan ".
					"ORDER by nama ASC";
	...
	while ($row = mysqli_fetch_assoc($empRecords)) 
		...
		$qyuk = mysqli_query($koneksi, "SELECT * FROM e_perilaku_masyarakat ".
											"WHERE tipe_laporan = '$i_kec' ".
											"AND postdate LIKE '$i_tglku%'");
		$ryuk = mysqli_fetch_assoc($qyuk);
		$yuk_total = mysqli_num_rows($qyuk);
		
		//update
		mysqli_query($koneksi, "UPDATE perilaku_lap_tipe_laporan SET nil$nomer = '$yuk_total' ".
									"WHERE kd = '$xyz'");
...
$sel = mysqli_query($koneksi,"select tanggal from perilaku_lap_tipe_laporan");
...
$sel = mysqli_query($koneksi,"select * from perilaku_lap_tipe_laporan ".
								"WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
...
## Fetch records
$empQuery = "select * from perilaku_lap_tipe_laporan ".
