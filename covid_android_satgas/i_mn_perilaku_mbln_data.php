...
while ($row2 = mysqli_fetch_assoc($empRecords2)) 
	...
	//insert
	$xyz = md5("$i_tglku");
	mysqli_query($koneksi, "INSERT INTO perilaku_lap_bln(kd, bulan, postdate) VALUES 
								('$xyz', '$i_tglku', '$today')");
	...
	//update
	mysqli_query($koneksi, "UPDATE perilaku_lap_bln SET jml_masker = '$yuk_masker_pake', ".
...
$sel = mysqli_query($koneksi,"select tanggal from perilaku_lap_bln");
...
$sel = mysqli_query($koneksi,"select * from perilaku_lap_bln ".
								"WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
...
## Fetch records
$empQuery = "select * from perilaku_lap_bln ".
