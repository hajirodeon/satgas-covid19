...
while ($row2 = mysqli_fetch_assoc($empRecords2)) 
	...
	//insert
	$xyz = md5("$i_tglku");
	mysqli_query($koneksi, "INSERT INTO perilaku_lap_kategori_tempat(kd, tanggal, postdate) VALUES 
								('$xyz', '$i_tglku', '$today')");


	
	//membaca semua data, trus masukin ke satu table
	$empQuery = "SELECT * FROM e_m_kategori_tempat ".
					"ORDER by nama ASC";
	...
	while ($row = mysqli_fetch_assoc($empRecords)) 
		...
		$qyuk = mysqli_query($koneksi, "SELECT * FROM e_perilaku_masyarakat ".
											"WHERE kategori_tempat = '$i_kec' ".
											"AND postdate LIKE '$i_tglku%'");
		$ryuk = mysqli_fetch_assoc($qyuk);
		$yuk_total = mysqli_num_rows($qyuk);
		
		//update
		mysqli_query($koneksi, "UPDATE perilaku_lap_kategori_tempat SET nil$nomer = '$yuk_total' ".
									"WHERE kd = '$xyz'");
...
$sel = mysqli_query($koneksi,"select tanggal from perilaku_lap_kategori_tempat");
...
$sel = mysqli_query($koneksi,"select * from perilaku_lap_kategori_tempat ".
								"WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
...
## Fetch records
$empQuery = "select * from perilaku_lap_kategori_tempat ".
