...
//nilai
$filenya = "$sumber/covid_android_satgas/i_mn_panic_mhistory_korban_data.php";
$filenyax = "$sumber/covid_android_satgas/i_mn_panic_mhistory_korban_data.php";
...
$sel = mysqli_query($koneksi,"select count(*) as allcount from umum_sesi_panic WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from umum_sesi_panic WHERE 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
...
while ($row = mysqli_fetch_assoc($empRecords)) {
    ...
    $data[] = array(	
    		"postdate"=>"$e_postdate",    		    		
    		"usernya"=>"$e_orang_kode . $e_orang_nama",
    		"gps"=>"$e_gps"
    	);
}
