...
while ($row2 = mysqli_fetch_assoc($empRecords2)) 
	...
	//insert
	$xyz = md5("$i_tglku");
	mysqli_query($koneksi, "INSERT INTO perilaku_lap_tipe_user(kd, tanggal, postdate) VALUES 
								('$xyz', '$i_tglku', '$today')");
	...
	while ($row = mysqli_fetch_assoc($empRecords)) 
		...
		$qyuk = mysqli_query($koneksi, "SELECT * FROM e_perilaku_masyarakat ".
											"WHERE kontributor_tipe = '$i_kec' ".
											"AND postdate LIKE '$i_tglku%'");
		$ryuk = mysqli_fetch_assoc($qyuk);
		$yuk_total = mysqli_num_rows($qyuk);
		
		//update
		mysqli_query($koneksi, "UPDATE perilaku_lap_tipe_user SET nil$nomer = '$yuk_total' ".
									"WHERE kd = '$xyz'");
...
$sel = mysqli_query($koneksi,"select tanggal from perilaku_lap_tipe_user");
...
$sel = mysqli_query($koneksi,"select * from perilaku_lap_tipe_user ".
								"WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
...
## Fetch records
$empQuery = "select * from perilaku_lap_tipe_user ".
