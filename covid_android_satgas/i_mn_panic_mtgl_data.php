...
while ($row2 = mysqli_fetch_assoc($empRecords2)) 
	...
	//insert
	$xyz = md5("$i_tglku");
	mysqli_query($koneksi, "INSERT INTO panic_lap_tgl(kd, tanggal, postdate) VALUES 
								('$xyz', '$i_tglku', '$today')");
	...
	//update
	mysqli_query($koneksi, "UPDATE panic_lap_tgl SET jml_korban = '$yuk_jml_korban', ".
...
$sel = mysqli_query($koneksi,"select tanggal from panic_lap_tgl");
...
$sel = mysqli_query($koneksi,"select * from panic_lap_tgl ".
								"WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
...
## Fetch records
$empQuery = "select * from panic_lap_tgl ".
