		...
		?>
			...
			$(document).ready(function(){
				...
				$("#kd<?php echo $i_kd;?>").on('click', function(){
					...
					$.ajax({
						url: "<?php echo $filenyax;?>?aksi=formedit&page6=<?php echo $page;?>&kd=<?php echo $i_kd;?>",
						type:$(this).attr("method"),
						data:$(this).serialize(),
						success:function(data){				
...
//jika update ya
if ((isset($_GET['aksi']) && $_GET['aksi'] == 'updateya'))
	{
	//ambil nilai
	$kd6 = cegah($_GET['kd6']);
	...
	//update
	mysqli_query($koneksi, "UPDATE $tabel2 SET orang_kode = '$e_kode6', ".
								"orang_nama = '$e_nama6' ".
								"WHERE kd = '$kd6'");
...
if ((isset($_GET['aksi']) && $_GET['aksi'] == 'formedit'))
	{
	//nilai
	$sort6 = cegah($_GET['sort6']);
	$cariku6 = cegah($_GET['cariku6']);
	$page6 = cegah($_GET['page6']);
	$kd = cegah($_GET['kd']);
	
	//totalnya
	$qyuk = mysqli_query($koneksi, "SELECT * FROM $tabel2 ".
										"WHERE kd = '$kd'");
	...
	$(document).ready(function(){
		...
		$("#btnKRM6").on('click', function(){
			
			
				$("#formx16").submit(function(){
					$.ajax({
						url: "<?php echo $filenyax;?>?aksi=updateya",
						type:$(this).attr("method"),
						data:$(this).serialize(),
						success:function(data){				
							$("#ihasil6").html(data);
							...
							$.ajax({
								url: "<?php echo $filenyax;?>?aksi=form&limitku6=<?php echo $limitku6;?>&sort6=<?php echo $sort6;?>&cariku6=<?php echo $cariku6;?>&page6=<?php echo $page6;?>&pageku="+pageku,
								type:$(this).attr("method"),
								data:$(this).serialize(),
								success:function(data){					
									$("#idetailtable6").html(data);
	...
	Kode : 
	<br>
	<input name="e_kode6" id="e_kode6" type="text" class="btn btn-block btn-warning" value="'.$yuk_kode.'" required>
	</p>
	<br>
	...
	Nama : 
	<br>
	<input name="e_nama6" id="e_nama6" type="text" class="btn btn-block btn-warning" value="'.$yuk_nama.'" required>
	</p>
	...
	<hr>
	<input name="kd6" id="kd6" type="hidden" value="'.$kd.'">
	<input name="page6" id="page6" type="hidden" value="'.$page6.'">
	
	
	<button id="btnKRM6" class="btn btn-danger"><i class="fa fa-save"></i> SIMPAN >></button>
