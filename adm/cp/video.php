...
if ($_POST['btnSMP'])
	...
	//nek null
	if ((empty($e_nama)) OR (empty($e_ket)))
		{
		//re-direct
		...
		if ($s == "baru")
			{
			//insert
			mysqli_query($koneksi, "INSERT INTO cp_video(kd, nama, filex, postdate) VALUES ".
							"('$e_kd', '$e_nama', '$e_ket', '$today')");
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE cp_video SET nama = '$e_nama', ".
							"filex = '$e_ket', ".
							"postdate = '$today' ".
							"WHERE kd = '$e_kd'");
...
if (($s == "baru") OR ($s == "edit"))
	...
	echo '<div class="row">
		...
		URL ALAMAT YOUTUBE : 
		<br>
		<input name="e_ket" type="text" size="30" value="'.$e_ket.'" class="btn btn-warning">
		</p>
    ...
    <thead>
			...
			<td><strong><font color="'.$warnatext.'">VIDEO YOUTUBE</font></strong></td>
			</tr>
	...
	if ($count != 0)
		...
		do {
			...
			<a href="'.$e_ket.'" title="'.$e_nama.'" target="_blank">'.$e_ket.'</a>
