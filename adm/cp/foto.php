...
if ($_POST['btnSMP'])
	...
	if (empty($e_nama))
		...
		if ($s == "baru")
			{
			//insert
			mysqli_query($koneksi, "INSERT INTO cp_foto(kd, nama, filex, postdate) VALUES ".
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE cp_foto SET nama = '$e_nama', ".
							"filex = '$namabaru', ".
							"postdate = '$today' ".
							"WHERE kd = '$e_kd'");
...
if (($s == "baru") OR ($s == "edit"))
	...
	$(document).ready(function(){
	       ...
	       $('#image-holder').load("<?php echo $sumber;?>/adm/cp/i_foto.php?aksi=lihat1&kd=<?php echo $kd;?>");
			...
			$('#loading').show();
		     ...
		     $.ajax({
		      ...
		      success:function(data){
				$('#loading').hide();
		       $('#preview').load("<?php echo $sumber;?>/adm/cp/i_foto.php?aksi=lihat&kd=<?php echo $kd;?>");
