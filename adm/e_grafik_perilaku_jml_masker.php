...
<h3 class="box-title">GRAFIK PERILAKU MASYARAKAT : PER JUMLAH PENGGUNAAN MASKER</h3>
</div>
...
for($i=0; $i<=7; $i++)
	...
	//ketahui ordernya...
	$qyuk = mysqli_query($koneksi, "SELECT SUM(jml_masker_pake) AS totalku ".
...
for($i=0; $i<=7; $i++)
	...
	//ketahui yg sukses
	$qyuk = mysqli_query($koneksi, "SELECT SUM(jml_masker_tidak_pake) AS totalku ".
							"FROM e_perilaku_masyarakat ".
							"WHERE round(DATE_FORMAT(postdate, '%d')) = '$itgl' ".
							"AND round(DATE_FORMAT(postdate, '%m')) = '$ibln' ".
							"AND round(DATE_FORMAT(postdate, '%Y')) = '$itahun'");
...
<script>
  $(function () {
    ...
    var areaChartCanvas = $('#areaChartp2_masker').get(0).getContext('2d')
    ...
    areaChart.Line(areaChartData, areaChartOptions)
				...
				?>
					...
					<div class="chart">
					<canvas id="areaChartp2_masker" style="height:250px"></canvas>
					</div>
				...
				//ketahui 
				$qyuk2 = mysqli_query($koneksi, "SELECT SUM(jml_masker_tidak_pake) AS totalku ".
										"FROM e_perilaku_masyarakat");
	            ...
	            <div class="card-footer clearfix">
	              <font color="orange"><b><?php echo $yuk_total;?></b> Memakai Masker</font>. 
	              
	              <font color="red"><b><?php echo $yuk2_total;?></b> Tidak Memakai Masker</font>.
	               
	              <a href="perilaku/grafik_perilaku_jml_masker.php" target="_parent" class="btn btn-sm btn-danger float-right">SELENGKAPNYA >></a>
	            </div>
