...
<h3 class="box-title">GRAFIK PANIC BUTTON : PER TANGGAL</h3>
</div>
...
for($i=0; $i<=7; $i++)
	...
	//ketahui ordernya...
	$qyuk = mysqli_query($koneksi, "SELECT DISTINCT(umum_kd) AS totalku ".
...
for($i=0; $i<=7; $i++)
	...
	$qyuk = mysqli_query($koneksi, "SELECT DISTINCT(umum_kd) AS totalku ".
							"FROM umum_sesi_panic ".
							"WHERE round(DATE_FORMAT(sukses_postdate, '%d')) = '$itgl' ".
							"AND round(DATE_FORMAT(sukses_postdate, '%m')) = '$ibln' ".
							"AND round(DATE_FORMAT(sukses_postdate, '%Y')) = '$itahun'");
	$tyuk = mysqli_num_rows($qyuk);
  ...
  $(function () {
    ...
    areaChart.Line(areaChartData, areaChartOptions)
	            ...
	            <div class="card-footer clearfix">
	              <font color="orange"><b><?php echo $tyuk;?></b> Kasus</font>. 
	              
	              <font color="red"><b><?php echo $tyuk2;?></b> Berhasil Ditangani</font>.
	               
	              <a href="k/grafik_panic_button_tgl.php" target="_parent" class="btn btn-sm btn-danger float-right">SELENGKAPNYA >></a>
	            </div>
