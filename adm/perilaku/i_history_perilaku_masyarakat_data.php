...
if($searchValue != ''){
	$searchQuery = " and (nama_lokasi like '%".$searchValue."%' or 
        ...
        jumlah_orang like '%".$searchValue."%' or 
        jml_masker_pake like '%".$searchValue."%' or 
        jml_masker_tidak_pake like '%".$searchValue."%' or 
        jml_jaga_jarak like '%".$searchValue."%' or 
        jml_jaga_jarak_tidak like '%".$searchValue."%' or 
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	$i_prank_ket = balikin($row['prank_ket']);
	...
	//jika prank, gak bisa aksi apapun
	if ($i_prank_ket == "PRANK")
		{
		$kakak_era = "PRANK/FAKE";
		}
		...
		$i_tugaskan_postdate = balikin($rkuy['tugaskan_postdate']);
	  	...
	  	if (!empty($tkuy))
			...
			'.$i_tugaskan_postdate.'
			<br>
			<a href="panggil_satgas.php?pkd='.$i_kd.'" target="_parent" class="btn btn-warning" title="LIHAT STATUS">LIHAT STATUS >></a>
			</p>';
			}
			
		else
			{
			$kakak_era = '<a href="panggil_satgas.php?pkd='.$i_kd.'" class="btn btn-success" title="Panggil SATGAS">PANGGIL SATGAS >></a>';
			}
		}
