...
//nilai
$filenya = "kategori_tempat.php";
$judul = "[PERILAKU] Kategori Tempat";
$judulku = "$judul";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if ($_POST['btnSMP'])
	...
	if (empty($e_nama))
		...
		if ($s == "baru")
			{
			//insert
			mysqli_query($koneksi, "INSERT INTO e_m_kategori_tempat(kd, nama, postdate) VALUES ".
							"('$e_kd', '$e_nama', '$today')");
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE e_m_kategori_tempat SET nama = '$e_nama', ".
							"postdate = '$today' ".
							"WHERE kd = '$e_kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM e_m_kategori_tempat ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	{
	//edit
	$qx = mysqli_query($koneksi, "SELECT * FROM e_m_kategori_tempat ".
						"WHERE kd = '$kd'");
	...
	$sqlcount = "SELECT * FROM e_m_kategori_tempat ".
					"ORDER BY nama ASC";
	$sqlresult = $sqlcount;
