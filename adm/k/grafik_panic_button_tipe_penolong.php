...
//nilai
$filenya = "grafik_panic_button_tipe_penolong.php";
$judul = "PER TIPE USER PENOLONG";
...
while ($row2 = mysqli_fetch_assoc($empRecords2)) 
	...
	//insert
	$xyz = md5("$i_tglku");
	mysqli_query($koneksi, "INSERT INTO panic_tipe_penolong(kd, tanggal, postdate) VALUES 
								('$xyz', '$i_tglku', '$today')");
	...
	while ($row = mysqli_fetch_assoc($empRecords)) 
		...
		//update
		mysqli_query($koneksi, "UPDATE panic_tipe_penolong SET nil$nomer = '$tjml_penolong' ".
									"WHERE kd = '$xyz'");
		}
...
//penolong tidak tertolong
$qyuk1 = mysqli_query($koneksi, "SELECT DISTINCT(penolong_kd) AS totalku ".
									"FROM umum_sesi_penolong ".
									"WHERE tugaskan = 'false' ".
									"WHERE postdate between '$tgl1_postdate%' AND '$tgl2_postdate%'");
$ryuk1 = mysqli_fetch_assoc($qyuk1);
$yuk1_penolong_tidak = mysqli_num_rows($qyuk1);
...
<input name="btnTPL" type="submit" value="TAMPILKAN >>" class="btn btn-danger">
	...
	<p>
          <font color="orange"><b>'.$yuk1_penolong.'</b> Orang penolong Tertolong</font>.
    </p>
    
	<p>
			<font color="red"><b>'.$yuk1_penolong_tidak.'</b> Orang penolong Tidak Tertolong</font>.
	</p>
...
<a id="link" download="grafik_panic_button_tipe_penolong-'.$e_tgl1.'-sampai-'.$e_tgl2.'.png" class="btn btn-success">UNDUH GRAFIK IMAGE >></a>



<a href="grafik_panic_button_tipe_penolong_xls.php?e_tgl1='.$e_tgl12.'&e_tgl2='.$e_tgl22.'" class="btn btn-danger">EXPORT EXCEL >></a>
<hr>
...
$(document).ready(function(){
    ...
    var table = $('#empTable').DataTable({
        ...
        'serverMethod': 'post',
        'ajax': {
            'url':'i_grafik_panic_button_tipe_penolong_data.php?e_tgl1=<?php echo $e_tgl12;?>&e_tgl2=<?php echo $e_tgl22;?>'
        },
        'columns': [
            { data: 'postdate' },
            { data: 'nil1' },
